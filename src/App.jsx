...
import { saveSession, loadSession, clearSession, fetchUsuarios, fetchConfig } from "./api.js";
...
const S = {
  ...
  header:    { background: "#1A2744", padding: "14px 16px 12px", position: "sticky", top: 0, zIndex: 50 },
  ...
  gearBtn:   { background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", fontSize: 18, width: 36, height: 36, borderRadius: 10, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  navItem:   (active) => ({ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "8px 4px 6px", background: "none", border: "none", cursor: "pointer", gap: 3 }),
  navIcon:   (active) => ({ fontSize: 22, lineHeight: 1 }),
...
export default function App() {
  ...
  const [module,     setModule]     = useState("lila");   // "lila"|"niveles"|"dash"|"dosis"
  ...
  // Cargar config de productos activos al montar
  useEffect(() => {
  ...
  return (
    <div style={S.app}>
      ...
      <header style={S.header}>
        <div style={S.headerRow}>
          <div>
            ...
            <div style={S.headerSub}>{usuario.nombre}{usuario.admin ? " ★" : ""}</div>
          </div>
      ...
      {module === "dash"    && <DashboardModule config={config} />}
      {module === "dosis"   && <DosificacionesModule usuario={usuario} showToast={showToast} />}
